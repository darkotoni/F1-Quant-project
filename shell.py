...
if __name__ == "__main__":
    ...
    # Get Shell stock data instead of Ferrari
    start_date = f"{season_start}-01-01"
    end_date = f"{season_end}-12-31"
    
    # Shell PLC ticker is SHEL (previously RDS-A before 2022 rebranding)
    shell_stock, shell_returns = get_stock_prices('SHEL', start_date, end_date)
    
    # If primary ticker fails, try alternate tickers
    if shell_stock.empty:
        print("Trying alternate ticker for Shell...")
        shell_stock, shell_returns = get_stock_prices('SHELL.L', start_date, end_date)  # London Stock Exchange
    
    if shell_stock.empty:
        print("Unable to retrieve Shell stock data.")
        exit(1)
    ...
    ax1.plot(shell_stock.index, shell_stock, label='Shell PLC (SHEL) Stock Price', color='#FFCC00')  # Shell yellow
    ...
    for i, end_date in enumerate(season_end_dates):
        ...
        try:
            closest_date = min(shell_stock.index, key=lambda x: abs((x - end_date).total_seconds()))
            price_at_season_end = shell_stock.loc[closest_date]
    ...
    try:
        closest_hamilton_date = min(shell_stock.index, key=lambda x: abs((x - hamilton_date).total_seconds()))
        price_at_signing = shell_stock.loc[closest_hamilton_date]
        
        # Add prominent vertical line for Hamilton's signing
        ax1.axvline(x=closest_hamilton_date, color='#FF2800', linestyle='-', linewidth=2, alpha=0.8)  # Ferrari red
        ...
        ax1.annotate(hamilton_text, xy=(closest_hamilton_date, price_at_signing), 
                     ...
                     arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=.2', color='#FF2800', linewidth=2),
        ...
        try:
            pre_5d = shell_stock.loc[:closest_hamilton_date].iloc[-6:-1]
            post_5d = shell_stock.loc[closest_hamilton_date:].iloc[:5]
    ...
    if isinstance(shell_returns, pd.DataFrame):
        returns_series = shell_returns.iloc[:, 0]
    else:
        returns_series = shell_returns
    ...
    ax2.plot(volatility.index, volatility.values, color='#FF2800', label='5-Day Rolling Volatility')  # Ferrari red
    ax2.fill_between(volatility.index, 0, volatility.values, color='#FF2800', alpha=0.2)
    ...
    ax1.set_title('Shell PLC (SHEL) Stock Price with Ferrari F1 Performance\nand Lewis Hamilton Signing', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('shell_ferrari_hamilton_analysis.png', dpi=300)
    plt.show()
